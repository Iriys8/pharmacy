...
import { createOrder, deleteOrder, getOrderByID, getOrders, updateOrder } from '@pharmacy/src/services/orders_service/controller';
import { connectBroker } from '@pharmacy/src/shared/setup/broker_connect';
import { OrderResponse } from '@pharmacy/src/shared/models/responses';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    Logger.info('Orders_service: Database connected');

    redisDB = await connectRedis();
    Logger.info('Orders_service: Redis connected');

    broker = await connectBroker();
    Logger.info('Orders_service: RabbitMQ connected');

    if (broker.channel) {
      consumeMessages(broker.channel, 'public_orders_queue', redisDB, dataSource, uname_public);
      consumeMessages(broker.channel, 'local_orders_queue', redisDB, dataSource, uname_local);
    }

    Logger.info('Orders_service: Service is running.');
  ...
  try {
    await ch.consume(queueName, async (msg: Message | null) => {
      ...
      try {
        ...
        Logger.info(`Orders_service: Received task: ${taskKey}`);

        const taskData = await redisDB.hgetall(taskKey);

        if (!taskData || Object.keys(taskData).length === 0) {
          Logger.info(`Orders_service: Task ${taskKey} not found in Redis`);
          ch.ack(msg);
          return;
        }

        if (taskData.status !== 'pending') {
          Logger.info(`Orders_service: Task ${taskKey} is not pending (status: ${taskData.status}), skipping`);
          ch.ack(msg);
          return;
        }
          ...
          switch (taskData.task) {
            case 'get': {
              ...
              if (!query.id || query.id === 0) {
                result = await getOrders(dataSource, query.q || '', query.page || '1', query.limit || '10', claims);
              } else {
                result = await getOrderByID(dataSource, query.id, claims);
              }
              break;
            }
            ...
            case 'post': {
              ...
              const createResult = await createOrder(dataSource, context as OrderResponse);
            ...
            case 'patch': {
              ...
              const updateResult = await updateOrder(dataSource,query.id, context as OrderResponse, claims as Claims);
            ...
            case 'delete': {
              ...
              const deleteResult = await deleteOrder(dataSource, query.id, claims as Claims);
              result = { Response: deleteResult };
              break;
            }

            default: {
              Logger.info(`Orders_service: Unknown task type: ${taskData.task}`);
              execError = new Error('Unknown task');
            }
          }
        } catch (err) {
          execError = err as Error;
          Logger.error(`Orders_service: Error:`, err)
        }
        ...
        Logger.info(`Orders_service: Task ${taskKey} completed successfully with status: ${taskData.status}`);
        ch.ack(msg);

      } catch (err) {
        Logger.error(`Orders_service: Error processing message:`, err);
        ch.ack(msg);
      }
    });
  } catch (err) {
    Logger.error(`Orders_service: Failed to register consumer for ${queueName}:`, err);
    throw err;
  }
}
